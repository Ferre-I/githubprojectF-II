...
GLfloat m_dif1[] = { 0.0, 0.2f, 1.0f, 1.0f };				// Diffuse Light Values
...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	objCamera.Strafe_Camera(-15);
	objCamera.Rotate_View(45);
...
void display ( void )   // Creamos la funcion donde se dibuja
{
	...
	glRotatef(g_lookupdown-30,1.0f,0,0);

		gluLookAt(	objCamera.mPos.x,  objCamera.mPos.y+40,  objCamera.mPos.z,	
...
void animacion()
{
	...
	if(circuito)
	{
		...
		{
			rotKit = -38.88;
			movKitX = movKitX  - sin(38.08);
			movKitZ = movKitZ  + cos(38.08);
			movKitY = movKitY + tan(0.4127930574);
			i++;
			printf("E y=%f %i\n", movKitY,i);
			if (movKitZ>-30)
			{
		...
		if (recorrido10)
		{
			...
			movKitX = movKitX - sin(38.08);
			movKitZ = movKitZ + cos(38.08);
			movKitY = movKitY - tan(0.4506613261);
			printf("F y=%f\n %i", movKitY, i);
			i++;
			if (movKitZ>155)
			{ 
...
void keyboard ( unsigned char key, int x, int y )  // Create Keyboard Function
{
	switch ( key ) {
		...
		case 'm':		//  
		case 'M':
			circuitoAtras ^= true; //Activamos/desactivamos la animacíon
			g_fanimacion = false;
			break;
